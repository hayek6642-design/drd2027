...
router.post('/sync', requireAuth, async (req, res) => {
  ...
  if (!userId) {
    console.error('[SYNC] No userId found in request');
    return res.status(401).json({ success: false, error: 'user_id_missing' })
  }
...
router.get('/', requireAuth, async (req, res) => {
  ...
  if (!userId) {
    console.error('[GET CODES] No userId found in request');
    return res.status(401).json({ success: false, error: 'user_id_missing' })
  }
  ...
  try {
    ...
    console.log('[GET CODES] Retrieved', result.rows.length, 'codes for user', userId)
